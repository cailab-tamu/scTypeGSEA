...
Next to use small data set "pbmc" go through this pipline. First is to load data set. If it is a gene expression count matrix, you can use "check_seurat" function to create a Seurat object.
```{r, message=FALSE}
library(scTypeGSEA)
library(Seurat)
pbmc_raw <- Read10X(data.dir = "~/Documents/single cell/package example/R package/Seurat/cluster/data set")
pbmc <- check_seurat(pbmc_raw, min.cells = 3, min.features = 200)
pbmc
```
...
```{r}
pbmc <- check_cluster(pbmc, percent.mt = 5,
  normalization.method = "LogNormalize", scale.factor = 10000, selection.method = "vst", nfeatures = 2000, npcs = 50,
